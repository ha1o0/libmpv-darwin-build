...
let
  ...
  mpvCrossFile =
    if macosNativeVideo then
      ...
      crossFile;
  macosSdkPath = "${pkgs.darwin.xcode}/Contents/Developer/Platforms/MacOSX.platform/Developer/SDKs/MacOSX.sdk";
  xcrun =
    if macosNativeVideo then
      pkgs.writeShellScriptBin "xcrun" ''
        ...
        if [ "$#" -eq 3 ] && [ "$1" = "--sdk" ] && [ "$2" = "macosx" ] && [ "$3" = "--show-sdk-version" ]; then
          ${pkgs.python3}/bin/python3 -c 'import plistlib, sys; print(plistlib.load(open(sys.argv[1], "rb"))["Version"])' "$sdk/SDKSettings.plist"
          exit 0
        fi
  ...
  patchedSource = pkgs.runCommand "${pname}-patched-source-${variant}-${version}" { } ''
    ...
    ${pkgs.lib.optionalString macosNativeVideo ''
      ...
      patch -p1 <${../../../patches/mpv-macos-embedded-macvk-async-init.patch}
    ''}
...
pkgs.stdenvNoCC.mkDerivation {
  ...
  configurePhase = ''
    ...
    ${pkgs.lib.optionalString macosNativeVideo ''
      # Swift's Cocoa bridging-header compile needs a writable Clang module cache.
      ...
      # Avoid mpv's sandbox-sensitive xcodebuild SDK-version probe.
      export MACOS_SDK="${macosSdkPath}"
      export MACOS_SDK_VERSION="$(${pkgs.python3}/bin/python3 -c 'import plistlib, sys; print(plistlib.load(open(sys.argv[1], "rb"))["Version"])' "$MACOS_SDK/SDKSettings.plist")"
    ''}
    DISABLE_ALL_OPTIONS=(
      `# booleans`
  ...
  installPhase = ''
    ...
    ${pkgs.lib.optionalString macosNativeVideo ''
      ...
      if ! lipo "$mpv_dylib" -verify_arch "$expected_arch"; then
